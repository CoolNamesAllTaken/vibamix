...
| Path | What |
|------|------|
| [src/](src/) | App code: `main.cpp`, `GUI.*`, `LEDStrip.*`, `MeshNode.*`, `mesh_model.*`, `image_xfer.*`, `app_config.*`, `mesh_keys.h`, `identity.*`, `ConfigMode.*`, `config_gatt.*`, `qr_screen.*`, `battery.*`, `badge_store.*` (raw-flash image slots), `dither.*` (2-bit→1bpp) |
...
|-----------|--------|------|
| mcuboot | `0x0` | 64K |
| image-0 (slot0) | `0x10000` | 600K |
| image-1 (slot1) | `0xa6000` | 600K |
| images | `0x13c000` | 64K (4 × 16K raw slots — [badge_store.c](src/badge_store.c)) |
| storage | `0x14c000` | 100K | settings (ZMS) — `chosen { zephyr,settings-partition }` |

OTA slots were shrunk from 664K to carve the `images` partition + a larger ZMS `storage` at the end
of flash (no DFU is wired up). A partition-layout change needs `west flash --erase`.
