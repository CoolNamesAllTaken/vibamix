...
| Path | What |
|------|------|
| [src/](src/) | App code: `main.cpp`, `GUI.*`, `LEDStrip.*`, `MeshNode.*`, `mesh_model.*`, `image_xfer.*`, `app_config.*`, `mesh_keys.h`, `identity.*`, `ConfigMode.*`, `config_gatt.*`, `qr_screen.*`, `battery.*` |
...
Build footprint: app image ≈ **25% of the code-flash region (~360 KB)**, **~38% RAM (~73 KB)**
